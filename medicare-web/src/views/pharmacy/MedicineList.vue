      ...
      <el-table v-loading="loading" :data="tableData" stripe border>
        ...
        <el-table-column prop="stock" label="库存" width="80">
          <template #default="{ row }">
            <span :style="{ color: row.stock <= row.safetyStock ? '#f56c6c' : '' }">{{ row.stock }}</span>
          </template>
        </el-table-column>
          ...
          <template #default="{ row }">
            <div class="action-buttons">
              <div class="action-row">
                <el-button size="small" type="success" style="min-width:52px" @click="openStockDialog(row, 'in')">入库</el-button>
                <el-button size="small" type="warning" style="min-width:52px" @click="openStockDialog(row, 'out')">出库</el-button>
              </div>
              <div class="action-row">
                <el-button size="small" type="primary" style="min-width:52px" @click="openMedDialog(row)">编辑</el-button>
                <el-popconfirm title="确定删除该药品? 删除后不可恢复" @confirm="handleDelete(row.id)">
                  <template #reference><el-button size="small" type="danger" style="min-width:52px">删除</el-button></template>
                </el-popconfirm>
              </div>
            </div>
          </template>
...
async function loadData() {
  loading.value = true
  try { const r = await listMedicines(keyword.value); tableData.value = r.data } catch {}
  loading.value = false
}

async function showLowStock() {
  loading.value = true
  try { const r = await listLowStock(); tableData.value = r.data } catch {}
  loading.value = false
}
...
async function handleDelete(id: number) {
  try { await deleteMedicine(id); ElMessage.success('删除成功'); loadData() } catch {}
}
