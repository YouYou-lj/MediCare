...
public abstract class BaseDAO<T> {
    ...
    private final BasicRowProcessor rowProcessor = new BasicRowProcessor(new JavaTimeBeanProcessor());
    ...
    protected T querySingle(String sql, Object... params) throws SQLException {
        ...
        try {
            ...
            return queryRunner.query(conn, sql, new BeanHandler<>(clazz, rowProcessor), params);
        } finally {
            closeQuietly(conn);
        }
    }
    ...
    protected List<T> queryList(String sql, Object... params) throws SQLException {
        ...
        try {
            conn = ConnectionConfig.getConnection();
            return queryRunner.query(conn, sql, new BeanListHandler<>(clazz, rowProcessor), params);
        } finally {
            closeQuietly(conn);
        }
    }
    ...
    /**
     * 执行插入并返回生成的主键
     * <p>MySQL BIGINT UNSIGNED 返回 BigInteger，需转换为 Long</p>
     */
    protected Long executeInsert(String sql, Object... params) throws SQLException {
        Connection conn = null;
        try {
            conn = ConnectionConfig.getConnection();
            Object result = queryRunner.insert(conn, sql, new ScalarHandler<>(), params);
            if (result instanceof Number) {
                return ((Number) result).longValue();
            }
            throw new SQLException("无法将主键转换为 Long，实际类型: " + (result == null ? "null" : result.getClass()));
        } finally {
            closeQuietly(conn);
        }
    }
